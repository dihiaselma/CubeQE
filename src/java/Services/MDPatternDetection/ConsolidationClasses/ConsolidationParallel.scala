  ...
  object ConsolidationParallel {
    ...
    def getModelsofModel(model: Model): mutable.HashMap[String, Model] = {
      ...
      for (resource <- resourceList) {
        ...
        if (!rdfNodeList.contains(resource)) {
          var visitedNodes = new mutable.LinkedHashSet[RDFNode]()
    ...
    def getModelOfResource(resource: Resource, model: Model, visitedNodes: mutable.LinkedHashSet[RDFNode]): Model = {
